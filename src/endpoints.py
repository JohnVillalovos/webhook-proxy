...
class Endpoint(object):
    ...
    def __init__(self, route, settings, action_metrics):
        ...
        with Endpoint.in_context(self):
            self._actions = list(
                Action.create(name, **(action_settings if action_settings else dict()))
    ...
    def accept(self):
        ...
        ) and self._accept_body(request.json, self._body)
    ...
    def _accept_headers(headers, rules):
        for key, rule in rules.items():
            ...
            if not re.match(translated_rule, translated_value):
                print(
                    'Failed to validate the "%s" header: "%s" does not match "%s"'
        ...
        for key, rule in rules.items():
            ...
            if isinstance(value, list):
                for idx, item in enumerate(value):
                    if not self._check_body(
                        item, rule, "%s.%s[%d]" % (prefix, key, idx)
                    ):
                        return False
    ...
    def _check_body(self, value, rule, property_path):
        if isinstance(rule, six.string_types):
            rule = Template(rule).render(read_config=docker_helper.read_configuration)

        if isinstance(value, six.string_types):
            value = Template(value).render(read_config=docker_helper.read_configuration)
        ...
        if isinstance(rule, dict) and isinstance(value, dict):
            ...
            print(
                'Failed to validate "%s": "%s" does not match "%s"'
