...
class DockerActionTest(ActionTestBase):
    ...
    def test_list(self):
        container = self.client.containers.run(
            image="alpine", command="sleep 10", detach=True
        )
    ...
    def test_execute(self):
        ...
        output = self._invoke(
            ...
            body={
                "incoming": {"image": "alpine", "command": 'echo "Hello from Docker"'}
            },
        )
    ...
    def test_embedded_arguments(self):
        ...
        container = self.client.containers.run(
            image="alpine", command="sleep 10", name=name, detach=True
        )
        ...
        try:
            output = self._invoke(
                ...
                body={
                    "incoming": {"pattern": "testing_", "status": {"value": "running"}}
                },
            )
                ...
                {"eval": {"block": '{% set _ = context.set("user", "nobody") %}'}},
                {
                    "docker": {
                        "$containers": {
                            "$run": {
                                "image": "alpine",
                                "command": 'sh -c "env && echo "user=$(whoami)""',
                                "user": "{{ context.user }}",
                                "remove": True,
                                "environment": [
                                    'UPPER={{ "upper"|upper }}',
    ...
    def test_images(self):
        output = self._invoke(
            {"docker": {"$images": {"$list": {"filters": {"reference": "alp*"}}}}}
        )
    ...
    def test_no_invocation(self):
        self.assertRaises(
            ConfigurationException, self._invoke, {"docker": {"output": "Uh-oh"}}
        )
        ...
        self._invoke(
            {
                "docker": {
                    "$containers": {
                        "$list": {"filters": {"name": "test", "unknown": 1}}
                    }
                }
            },
