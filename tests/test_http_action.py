...
class HttpActionTest(ActionTestBase):
    ...
    def test_simple_http(self):
        output = self._invoke_http(
            headers={"X-Test": "Hello"}, body=json.dumps({"key": "value"})
        )
    ...
    def test_target_replacement(self):
        output = self._invoke_http(target='/some/{{ "remote" }}/endpoint')

        self.assertIn("uri=/some/remote/endpoint", output)

    def test_header_replacement(self):
        output = self._invoke_http(headers={"X-Original-Path": "{{ request.path }}"})
    ...
    def test_json_output_with_replacement(self):
        output = self._invoke_http(
            json=True, body={"original": {"request": {"path": "{{ request.path }}"}}}
        )
    ...
    def test_output_formatting(self):
        output = self._invoke_http(output="HTTP::{{ response.status_code }}")
    ...
    def test_content_length_header(self):
        ...
        output = self._invoke_http(headers={"Content-Type": "text/plain"}, body=message)
    ...
    def test_fail_on_error(self):
        with capture_stream("stderr") as stderr:
            output = self._invoke_http(
                headers={"X-Fail": "500"}, fail_on_error=True, expected_status_code=500
            )

            error = stderr.dumps()

            self.assertIn(
                "ActionInvocationException: HTTP call failed (HTTP 500)", error
            )
