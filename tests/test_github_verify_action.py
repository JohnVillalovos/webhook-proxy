...
class ExecuteActionTest(ActionTestBase):
    ...
    def test_successful_github_webhook(self):
        self._headers[
            "X-Hub-Signature"
        ] = "sha1=28fdad22dac5d0a631b5ead69dbb05e43b685d6e"

        with open(
            os.path.join(os.path.dirname(__file__), "github/webhook.json")
        ) as webhook:
            output = self._invoke(
                {"github-verify": {"secret": "TopSecret"}}, final_body=webhook.read()
            )
    ...
    def test_invalid_github_hash_algorithm(self):
        self._headers[
            "X-Hub-Signature"
        ] = "md5=a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0a0"

        with open(
            os.path.join(os.path.dirname(__file__), "github/webhook.json")
        ) as webhook:
            self._invoke(
                {"github-verify": {"secret": "TopSecret"}},
    ...
    def test_invalid_github_secret(self):
        ...
        with open(
            ...
            self._invoke(
                {"github-verify": {"secret": "ThisIsNotRight"}},
                final_body=webhook.read(),
                expected_status_code=500,
            )
