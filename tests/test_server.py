...
class ServerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.server = Server(
            [
                {
                    "/testing": {
                        "headers": {"X-Sample": "^ab[0-9]+$"},
                        "body": {"key": "value", "item": {"prop": "^[0-9]*$"}},
                    }
                }
            ]
        )
    ...
    def test_valid_request(self):
        headers = {"X-Sample": "ab001"}

        body = {"key": "value", "item": {"prop": "123"}}
    ...
    def test_valid_request_with_list(self):
        ...
        body = {
            "key": "value",
            "item": [{"prop": "001"}, {"prop": "002"}, {"prop": "999"}],
        }
    ...
    def test_valid_request_with_templates(self):
        ...
        headers = {"X-Sample": '{{ "AB001"|lower }}'}

        body = {"key": "value", "item": {"prop": '{{ read_config("PROP_FROM_ENV") }}'}}
    ...
    def test_valid_request_with_view_args(self):
        ...
        with capture_stream() as sout:
            response = self.client.post(
                "/testing/1/abc/def/ghi", data="{}", content_type="application/json"
            )
    ...
    def test_invalid_body_second_level(self):
        ...
        body = {"key": "value", "item": {"prop": "invalid"}}
    ...
    def test_invalid_body_with_list(self):
        ...
        body = {
            "key": "value",
            "item": [{"prop": "001"}, {"prop": "002"}, {"prop": "notanumber"}],
        }
    ...
    def test_non_json_request(self):
        response = self.client.post(
            "/testing", data="plain text", content_type="text/plain"
        )
    ...
    def test_get_request(self):
        ...
        server = Server(
            [{"/get": {"method": "GET", "headers": {"X-Method": "(GET|HEAD)"}}}]
        )
    ...
    def test_validation_with_templates(self):
        ...
        headers = {"X-Test": "templated-header"}

        body = {"key": "templated-body"}
    ...
    def test_metrics(self):
            ...
            [
                {
                    "/test/post": {
                        "async": True,
                        "actions": [{"sleep": {"seconds": 0.01}}, {"log": {}}],
                    },
                    "/test/put": {
                        "method": "PUT",
                        "actions": [
        ...
        self.assertIn(
            "flask_http_request_total{" 'method="POST",status="200"} 2.0', metrics
        )
        self.assertIn(
            "flask_http_request_total{" 'method="PUT",status="200"} 3.0', metrics
        )

        self.assertIn(
            "flask_http_request_duration_seconds_bucket{"
            'le="5.0",method="POST",path="/test/post",status="200"} 2.0',
            metrics,
        )
        self.assertIn(
            "flask_http_request_duration_seconds_count{"
            'method="POST",path="/test/post",status="200"} 2.0',
            metrics,
        )
        self.assertIn(
            "flask_http_request_duration_seconds_sum{"
            'method="POST",path="/test/post",status="200"}',
            metrics,
        )

        self.assertIn(
            "flask_http_request_duration_seconds_bucket{"
            'le="0.5",method="PUT",path="/test/put",status="200"} 3.0',
            metrics,
        )
        self.assertIn(
            "flask_http_request_duration_seconds_count{"
            'method="PUT",path="/test/put",status="200"} 3.0',
            metrics,
        )
        self.assertIn(
            "flask_http_request_duration_seconds_sum{"
        ...
        self.assertIn(
            "webhook_proxy_actions_count{"
            'action_index="0",action_type="sleep",'
            'http_method="POST",http_route="/test/post"} 2.0',
            metrics,
        )
        self.assertIn(
            "webhook_proxy_actions_sum{"
            'action_index="0",action_type="sleep",'
            'http_method="POST",http_route="/test/post"}',
            metrics,
        )
        self.assertIn(
            "webhook_proxy_actions_count{"
        ...
        self.assertIn(
            ...
            'http_method="PUT",http_route="/test/put"} 3.0',
            metrics,
        )
        self.assertIn(
            "webhook_proxy_actions_sum{"
            'action_index="0",action_type="log",'
            'http_method="PUT",http_route="/test/put"}',
            metrics,
        )
        self.assertIn(
            "webhook_proxy_actions_count{"
